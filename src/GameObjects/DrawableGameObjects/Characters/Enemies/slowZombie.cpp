...
namespace ph {
	...
	SlowZombie::SlowZombie(GameData* gameData)
		:Enemy(gameData, name, animation, static_cast<unsigned int>(movementSpeed), hp, maxHp, posAndSize, mass, damage)
		, mIsSlownDown(false)
	{
		mSprite.setTexture(gameData->getTextures().get("textures/characters/zombieHeavyFullAnimation.png"));
	...
	void SlowZombie::updateCurrent(sf::Time delta)
	{
		if (isDead()) {
	...
	void SlowZombie::handlePlayerHit()
	{
	...
	void SlowZombie::move(sf::Time delta)
	{
		setPosition(mCollisionBody.getFixedPosition());
	...
	void SlowZombie::updateAnimation(sf::Time delta)
	{
		if (mIsSlownDown)
